...
const items = [
	...
	{
		icon: 'barometer',
		alt: 'A barometer with a brown case and white clock face',
	},
	{ icon: 'bell', alt: 'A golden bell' },
	{
		icon: 'captain',
		alt:
			'A captain with a whiteblue-white captions hat and a white uniform',
	},
	...
	{ icon: 'lifesaver', alt: 'A red-white lifesaver ring' },
	{
		icon: 'map',
		alt:
			'A treasure map that shows land and water and an x that marks the treassure',
	},
...
function openBarrel( element ) {
	...
	const item = items.find( ( thisElement ) => {
		return (
			thisElement.icon ==
			element.parentElement.getAttribute( 'data-item' )
		);
	} );
